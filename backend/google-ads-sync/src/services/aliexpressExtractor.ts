export interface AliExpressCost {
  value: number;
  currency: string;
}

export interface AliExpressExtractResult {
  canonicalUrl: string;
  productId: string;
  title: string;
  cost: AliExpressCost | null;
  functions: string[];
  sources: string[];
  warnings: string[];
}

function extractProductIdFromUrl(parsed: URL): string {
...
function normalizeCurrency(value: string): string {
  ...
  if (/^[A-Z]{3}$/.test(curr)) return curr;
  return 'USD';
}
...
export function normalizeAliExpressUrl(url: string): URL | null {
  let parsed: URL;
  try {
    parsed = new URL(url);
  } catch {
  ...
  if (!hostname.includes('aliexpress.com')) return null;
...
function extractFromPdpNpi(url: URL): AliExpressCost | null {
...
function extractPriceFromHtml(html: string): AliExpressCost | null {
...
function extractFunctionsFromHtml(html: string): string[] {
  ...
  for (const item of allMatches) {
    ...
    if (/^(home|help|shopping|feedback|privacy|terms|copyright)/i.test(text)) continue;
    lines.push(text);
...
function deriveFunctionsFromTitle(title: string): string[] {
...
export async function extractAliExpressData(url: string): Promise<AliExpressExtractResult> {
  ...
  const productId = extractProductIdFromUrl(parsed);
  const result: AliExpressExtractResult = {
