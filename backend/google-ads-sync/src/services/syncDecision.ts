import { QueuePayload, QueueRow, SyncDecision } from '../types.js';

export function computeSyncDecision(row: QueueRow, nextHash: string): SyncDecision {
...
function parseJsonArray(raw: string, label: string, errors: string[]): string[] {
...
export function buildPayloadFromRow(row: QueueRow): { payload: QueuePayload | null; errors: string[] } {
  const errors: string[] = [];
  ...
  if (desired === 'READY') {
    if (!customerId) errors.push('CustomerID é obrigatório para READY.');
    if (!campaignId) errors.push('CampaignID é obrigatório para READY.');
    if (!adGroupId) errors.push('AdGroupID é obrigatório para READY.');
    if (!adName) errors.push('AdName é obrigatório para READY.');
    if (!finalUrl) errors.push('FinalURL é obrigatório para READY.');
    if (headlines.length === 0) errors.push('HeadlinesJSON precisa de ao menos 1 item.');
    if (descriptions.length === 0) errors.push('DescriptionsJSON precisa de ao menos 1 item.');
    if (marketingImageAssetIds.length === 0) errors.push('MarketingImageAssetIDsJSON precisa de ao menos 1 item.');
    if (squareMarketingImageAssetIds.length === 0) errors.push('SquareMarketingImageAssetIDsJSON precisa de ao menos 1 item.');
  }
